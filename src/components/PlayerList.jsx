...
export function PlayerRow({ name, id, onClick, element, conn, me, showId, showOnline, ready }) {
    ...
    return (
        <div onClick={onClick ? () => onClick(id) : () => { }} className={'bg-neutral-content overflow-hidden rounded-md h-12 skew p-2 pl-6 text-title font-extrabold text-sm w-full flex justify-between items-center' + (onClick ? " clickable " : "")}>
